...
void draw() {
  ...
  if (z && !myTurn) {
    grid[pr1][pc1] = prev1;
    grid[pr2][pc2] = prev2;
    myServer.write("z" + pr1 + pc1 + pr2 + pc2 + prev1 + prev2);
    z = false;
    myTurn = true;
  }
  for (int c=0; c<8; c++) {
...
void mouseReleased() {
  if (firstClick || Character.isLowerCase(grid[mouseY/100][mouseX/100])) {
    row1 = mouseY/100;
    col1 = mouseX/100;
    firstClick = false;
  } else {
    ...
    if (myTurn && !(row2 == row1 && col2 == col1)) {
      ...
      myTurn =false;
      firstClick = true;
      myServer.write("m" + row2 + col2 + row1 + col1);
    }
  }
}
...
void keyPressed() {
  if ((key=='z' || key=='Z') && !myTurn) { 
    ...
    pressed = true;
    promote = Character.toLowerCase(key);
  }
}
